...
function [train_acc,test_acc,Rate,Error,O2,T2]=BallMill_Classification_SCN()
format long;
load('class_SY_GLP');

T = Tools.LabelToMatrix(T,3);
T2 = Tools.LabelToMatrix(T2,3);
%% show some samples
% X: each row vector represents one sample input.
% T: each row vector represents one sample label.
% same to the X2 and T2.
n1 = randperm(40, 5); % Randomly select 5 samples from training set.
disp('Training Input:')
disp(X(n1,:))
disp('Training Label:')
disp(T(n1,:))

n2 = randperm(40, 2);  % Randomly select 2 samples from test set.
disp('Test Input:')
disp(X2(n2,:))
disp('Test Label:')
disp(T2(n2,:))
...
%% Parameter Setting
L_max =300;                    % maximum hidden node number
tol = 0.01;                    
% training tolerance
T_max = 300;                  % maximun candidate nodes number
Lambdas = [1:1:1000];       % scope sequence
r =  [0.9, 0.99, 0.999, ...
...
[M, per] = M.Classification(X, T);% �������ݽ���ģ��

% [M, O, E, Error] = M.UpgradeSCN(X, T); % ֮�����ݸ�������
...
test_acc=M.GetAccuracy(X2, T2);
figure;
plotconfusion(T2',O2','Test  ');
Error = single(per.Error)
Rate = single(per.Rate)
end
